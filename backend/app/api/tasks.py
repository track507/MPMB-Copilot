...
async def list_tasks(limit: int = 20):
    ...
    return {"tasks": [t.to_dict() for t in tasks], "total": len(task_manager.tasks)}
...
async def cancel_task(task_id: str):
    ...
    if task_info.status not in (TaskStatus.PENDING, TaskStatus.RUNNING):
        raise HTTPException(status_code=400, detail=f"Cannot cancel task with status: {task_info.status}")
    ...
    if cancelled:
        ...
        raise HTTPException(status_code=400, detail="Task could not be cancelled (may have already completed)")
