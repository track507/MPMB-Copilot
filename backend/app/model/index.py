...
class IndexRequest(BaseModel):
	...
	source_path: Optional[str] = Field(
		None,
		description="Path to source files (default: configured MPMB source path)"
	)
	force_reindex: bool = Field(
		False,
		description="Force re-indexing all files regardless of modification status"
	)
	chunk_size: Optional[int] = Field(
		None,
		gt=100,
		le=5000,
		description="Chunk size in characters (100-5000, default: ~1500)"
	)
	chunk_overlap: Optional[int] = Field(
		None,
		ge=0,
		le=1000,
		description="Overlap between chunks in characters (0-1000, default: ~200)"
	)
...
class IndexResponse(BaseModel):
	...
	task_id: Optional[str] = Field(
		None,
		description="Task ID for async operations (use for status polling)"
	)
