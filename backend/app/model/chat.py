...
class ChatResponse(BaseModel):
	...
	sources: Optional[list[dict]] = Field(
		None,
		description="List of source code references used in generation"
	)
	metadata: dict = Field(
		...,
		description="Generation metadata (model, tokens, timing, retrieval info)"
	)
...
class ChatStreamChunk(BaseModel):
	...
	metadata: Optional[dict] = Field(
		None,
		description="Generation metadata (only in final chunk when done=True)"
	)
