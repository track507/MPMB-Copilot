...
class File(Base):
	...
	id: UUID = Column(PGUUID(as_uuid=True), primary_key=True, default=uuid7)
	session_id: UUID = Column(PGUUID(as_uuid=True), ForeignKey("sessions.id", ondelete="CASCADE"), nullable=False)
	message_id: Optional[UUID] = Column(PGUUID(as_uuid=True), ForeignKey("messages.id", ondelete="CASCADE"), nullable=True)
...
class MessageRetrieval(Base):
	...
	document_chunk_id: UUID = Column(PGUUID(as_uuid=True), ForeignKey("document_chunks.id", ondelete="CASCADE"), nullable=False)
