...
class CodeChunk(BaseModel):
    ...
    content: str = Field(..., description="Code text content for this chunk")
    source_file: str = Field(
        ..., description="Relative path to source file (e.g., 'src/common functions/SpellsList.js')"
    )
    chunk_index: int = Field(..., ge=0, description="Zero-based index of chunk within source file")
    start_line: int = Field(..., ge=1, description="Starting line number in source file (1-based)")
    end_line: int = Field(..., ge=1, description="Ending line number in source file (1-based, inclusive)")
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional chunk metadata (function names, types, complexity, etc.)"
    )
...
class EmbeddingRequest(BaseModel):
    ...
    texts: list[str] = Field(..., min_length=1, description="List of texts to embed (code chunks or queries)")
    model: Optional[str] = Field(None, description="Embedding model to use (default: configured model)")
...
class EmbeddingResponse(BaseModel):
    ...
    Note:
            ...
            - 1536 dimensions x 4 bytes (float32) = ~6KB per vector
    ...
    dimension: int = Field(..., gt=0, description="Dimensionality of each embedding vector")
...
class VectorSearchResult(BaseModel):
    ...
    id: str = Field(..., description="Unique chunk ID in vector database (matches DocumentChunk.qdrant_id)")
    score: float = Field(..., ge=0.0, le=1.0, description="Similarity score (0.0-1.0, higher = more relevant)")
    content: str = Field(..., description="Retrieved code chunk text")
    metadata: dict[str, Any] = Field(
...
class RAGContext(BaseModel):
    ...
    total_tokens: int = Field(..., ge=0, description="Estimated tokens in retrieved context (chunks + query)")
    context_window_used: float = Field(
        ..., ge=0.0, le=1.0, description="Percentage of LLM context window used by retrieved context (0.0-1.0)"
    )
...
class RetrievalMetadata(BaseModel):
    ...
    query: str = Field(..., description="Query text that was searched")
    top_k: int = Field(..., gt=0, description="Number of results requested from search")
    similarity_threshold: float = Field(
        ..., ge=0.0, le=1.0, description="Minimum similarity score for results (0.0-1.0)"
    )
    results_found: int = Field(..., ge=0, description="Actual number of results returned above threshold")
    retrieval_time_ms: float = Field(..., ge=0.0, description="Total retrieval time in milliseconds")
