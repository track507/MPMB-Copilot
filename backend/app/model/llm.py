...
class LLMProvider(BaseModel):
    ...
    api_key: Optional[str] = Field(None, description="API authentication key (not needed for Ollama)")
    base_url: Optional[str] = Field(
        None, description="Custom API endpoint (required for Ollama, optional for cloud providers)"
    )
    default_model: str = Field(..., description="Default model name to use for this provider")
...
class LLMRequest(BaseModel):
    ...
    model: str = Field(..., description="Model name to use (provider-specific)")
    temperature: float = Field(
        0.2, ge=0.0, le=2.0, description="Sampling temperature (0.0=deterministic, 2.0=creative)"
    )
    max_tokens: int = Field(4000, gt=0, description="Maximum tokens to generate in response")
    stream: bool = Field(False, description="Enable streaming response (returns LLMStreamChunk objects)")
    system_prompt: Optional[str] = Field(None, description="Optional system prompt to prepend (adds system message)")
...
class LLMResponse(BaseModel):
    ...
    provider: str = Field(..., description="Provider that generated the response (anthropic/openai/ollama)")
    usage: dict[str, int] = Field(..., description="Token usage stats (prompt_tokens, completion_tokens, total_tokens)")
    stop_reason: Optional[str] = Field(None, description="Why generation stopped (end_turn/max_tokens/stop_sequence)")
...
class LLMStreamChunk(BaseModel):
    ...
    content: str = Field(..., description="Partial response text for this chunk")
    done: bool = Field(False, description="Whether this is the final chunk in the stream")
    model: Optional[str] = Field(None, description="Model name (only in final chunk when done=True)")
    finish_reason: Optional[str] = Field(
        None, description="Stream completion reason (only in final chunk when done=True)"
    )
