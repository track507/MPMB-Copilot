...
from app.core.tools import validate_client
from app.core.tools.validate_client import run_validator
...
def _patch_spawn(monkeypatch, proc: FakeProc) -> None:
    ...
    monkeypatch.setattr(validate_client.asyncio, "create_subprocess_exec", fake_exec)
...
async def test_timeout_degrades(monkeypatch):
    monkeypatch.setattr(validate_client.config, "validator_timeout_sec", 0.01)
...
async def test_missing_script_degrades(monkeypatch):
    monkeypatch.setattr(validate_client.config, "validator_script_path", "./does/not/exist.mjs")
    result = await run_validator("var x;", "2014")
...
async def test_missing_node_binary_degrades(monkeypatch):
    ...
    monkeypatch.setattr(validate_client.asyncio, "create_subprocess_exec", raise_missing)
    result = await run_validator("var x;", "2014")
